...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!result.success) {
      return toast.error(result.message || "Đăng nhập thất bại");
    }
  ...
  return (
    <div className="card p-8 animate-slide-up">
      ...
      <p className="text-sm text-fg-muted mb-6">
        Hãy trải nghiệm những điều tuyệt vời nhất
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? <Spinner className="w-4 h-4" /> : "Đăng nhập"}
        </button>
      </form>

      <div className="relative my-5">
        <div className="divider my-0" />
        <span className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 bg-bg-elevated px-2 text-sm text-fg-subtle">
          hoặc
        </span>
      </div>
